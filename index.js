    ...
    switch (view) {
      case "Services":
        axios
          // Get request to retrieve the current weather data using the API key and providing a city name
          .get(
            `https://api.openweathermap.org/data/2.5/weather?appid=${process.env.OPEN_WEATHER_MAP_API_KEY}&q=st%20louis`//need to insert api url for this custom form. How do I get?//
          )
          .then(response => {
            console.log(response.data);
            // Convert Kelvin to Fahrenheit since OpenWeatherMap does provide otherwise
